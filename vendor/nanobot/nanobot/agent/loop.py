...
class AgentLoop:
    ...
    def __init__(
        ...
        memory_consolidation_max_tokens: int | None = None,
    ):
    ...
    def _register_default_tools(self) -> None:
        ...
        # Register ask_agent tool for inter-agent synchronous conversations
        try:
    ...
    def _unpack_agent_loop_result(
        ...
        raise ValueError(
            f"_run_agent_loop() must return a 3-tuple or 4-tuple, got {len(result)} values"
        )
    ...
    async def _run_agent_loop(
        ...
        while iteration < self.max_iterations:
            ...
            logger.info(
                "[agent:{}] calling model={} (iteration={})", self.agent_name, self.model, iteration
            )
            ...
            if response.has_tool_calls:
                ...
                if response.finish_reason == "error":
                    ...
                    error_message = clean or "Sorry, I encountered an error calling the AI model."
    ...
    def _get_consolidation_lock(self, session_key: str) -> asyncio.Lock:
        """Get or create the consolidation lock for a session."""
    ...
    def _consolidation_files_touched(self) -> list[str]:
        return [
            str(self.memory_workspace / "memory" / "MEMORY.md"),
            str(self.memory_workspace / "memory" / "HISTORY.md"),
        ]
    ...
    def _log_consolidation_event(
        ...
        *,
        ...
        payload = {
            ...
            "artifacts_workspace": str(self.workspace),
            "action": action,
            "skip_reason": skip_reason,
            "files_touched": files_touched or [],
        }
        logger.info("memory.consolidation {}", json.dumps(payload, sort_keys=True))

    def _maybe_schedule_threshold_consolidation(self, session: Session, *, channel: str) -> None:
        ...
        if session.key in self._consolidating:
            self._log_consolidation_event(
                ...
                boundary_reason=None,
                action="skipped",
                skip_reason="already_in_progress",
            )
            return
        ...
        async def _consolidate_and_unlock() -> None:
            try:
                async with lock:
                    snapshot = Session(key=session.key)
                    snapshot.messages = list(session.messages)
                    snapshot.last_consolidated = session.last_consolidated
                    ok = await self._consolidate_memory(snapshot)
                    if not ok:
                        self._log_consolidation_event(
                            channel=channel,
                            trigger_type="threshold",
                            boundary_reason=None,
                            action="skipped",
                            skip_reason="consolidate_returned_false",
                        )
                        return
                    ...
                    self._log_consolidation_event(
                        ...
                        boundary_reason=None,
                        action="consolidated",
                        files_touched=self._consolidation_files_touched(),
                    )
            except Exception:
                self._log_consolidation_event(
                    channel=channel,
                ...
                logger.exception("threshold memory consolidation failed for {}", session.key)
            finally:
    ...
    async def _process_message(
        ...
        self._save_turn(session, all_msgs, 1 + len(history))
        self.sessions.save(session)
        self._maybe_schedule_threshold_consolidation(session, channel=msg.channel)
                ...
                if snapshot:
                    ...
                    if not await self._consolidate_memory(temp, archive_all=True):
                        self._log_consolidation_event(
                            channel="mc",
                            trigger_type="session_boundary",
                            boundary_reason="task_completion",
                            action="skipped",
                            skip_reason="consolidate_returned_false",
                        )
                        logger.warning(
