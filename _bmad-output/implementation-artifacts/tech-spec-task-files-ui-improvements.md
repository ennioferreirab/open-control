...
- [x] **Task 1 — Backend: add `removeTaskFile` mutation to `convex/tasks.ts`**
...
- [x] **Task 2 — API: add `DELETE` handler to `app/api/tasks/[taskId]/files/route.ts`**
...
- [x] **Task 3 — UI: update `TaskDetailSheet.tsx`**
...
All 3 tasks implemented on 2026-02-23. TypeScript check passed clean.

- **Task 1**: Added `removeTaskFile` mutation to `convex/tasks.ts` after `addTaskFiles`. Filters `task.files` by excluding the matching `name + subfolder` entry and patches the document.
- **Task 2**: Added `DELETE` export to `app/api/tasks/[taskId]/files/route.ts`. Validates `taskId` (existing regex), parses JSON body, enforces `subfolder === "attachments"`, sanitizes filename via `basename()` to prevent path traversal, calls `unlink()` with ENOENT treated as success.
- **Task 3**: Updated `TaskDetailSheet.tsx` — added `Trash2` import, `removeTaskFile` mutation hook, `deletingFiles: Set<string>` state, `handleDeleteFile` async handler, and replaced the files tab JSX with always-visible ATTACHMENTS and OUTPUTS sections. Badge removed from file rows; `min-w-0 truncate` applied to filename span; trash icon hidden by default, revealed on hover via Tailwind `group/group-hover`.

Badge import retained (still used in Config tab for reviewers/tags and in the sheet header status badge).

## File List

- `dashboard/convex/tasks.ts` — added `removeTaskFile` mutation
- `dashboard/app/api/tasks/[taskId]/files/route.ts` — added `DELETE` handler, added `unlink` + `basename` imports
- `dashboard/components/TaskDetailSheet.tsx` — added `Trash2` import, `removeTaskFile` hook, `deletingFiles` state, `handleDeleteFile` handler, replaced files tab JSX

## Change Log

- 2026-02-23: Implemented Task Files UI improvements — removed redundant badge, fixed long-filename truncation, added attachment delete (disk + Convex metadata), always-visible sections with per-section empty states.
- 2026-02-23: Code review fixes — added subfolder guard to `removeTaskFile` mutation (H1), runtime field validation in DELETE handler (M1), `deleteError` state + UI feedback for failed deletes (M2).
...
**Total Action Items:** 3 High+Medium (all resolved), 2 Low (documented)

### Action Items

- [x] [High] `removeTaskFile` mutation accepted any `subfolder` — output metadata could be deleted directly. Added `if (subfolder !== "attachments") return;` guard. (`convex/tasks.ts`)
- [x] [Medium] Missing runtime validation for `filename`/`subfolder` fields in DELETE handler — `basename(undefined)` would throw → 500. Added `typeof` guard before `basename` call. (`route.ts`)
- [x] [Medium] Silent error swallowing in `handleDeleteFile` — no user feedback on failure. Added `deleteError` state + error message displayed in Files tab. (`TaskDetailSheet.tsx`)
- [ ] [Low] Misleading catch comment — says "Convex state will reflect truth" which is incorrect if disk DELETE succeeds but Convex throws. Updated comment to reflect retry-idempotency model.
- [ ] [Low] `filter` in `removeTaskFile` removes ALL duplicate entries — should only remove first match. Acceptable given upload behaviour prevents duplicates in practice.
