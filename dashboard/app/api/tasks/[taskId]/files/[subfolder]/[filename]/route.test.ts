...
function makeReq(taskId: string, subfolder: string, filename: string) {
  return new NextRequest(`http://localhost/api/tasks/${taskId}/files/${subfolder}/${filename}`);
}
...
describe("GET /api/tasks/[taskId]/files/[subfolder]/[filename]", () => {
  ...
  it("returns 200 with Content-Type application/pdf for .pdf", async () => {
    ...
    const res = await GET(
      makeReq("task-1", "output", "report.pdf"),
      makeParams("task-1", "output", "report.pdf"),
    );
  ...
  it("returns 200 with Content-Type text/markdown for .md", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "output", "README.md"),
      makeParams("task-1", "output", "README.md"),
    );
    expect(res.status).toBe(200);
  ...
  it("returns 200 with Content-Type text/x-python for .py", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "output", "script.py"),
      makeParams("task-1", "output", "script.py"),
    );
    expect(res.status).toBe(200);
  ...
  it("returns 200 with Content-Type application/json for .json", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "output", "data.json"),
      makeParams("task-1", "output", "data.json"),
    );
    expect(res.status).toBe(200);
  ...
  it("returns 200 with Content-Type image/png for .png", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "attachments", "photo.png"),
      makeParams("task-1", "attachments", "photo.png"),
    );
    expect(res.status).toBe(200);
  ...
  it("includes Content-Disposition with encoded filename", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "output", "my file.pdf"),
      makeParams("task-1", "output", "my file.pdf"),
    );
  ...
  it("sets Content-Length to buffer byte count", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "output", "note.txt"),
      makeParams("task-1", "output", "note.txt"),
    );
    expect(res.status).toBe(200);
  ...
  it("sets Cache-Control to private, max-age=60", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "output", "file.txt"),
      makeParams("task-1", "output", "file.txt"),
    );
    expect(res.status).toBe(200);
  ...
  it("returns 404 with { error: 'File not found' } when file does not exist (ENOENT)", async () => {
    ...
    const res = await GET(
      makeReq("task-1", "output", "missing.txt"),
      makeParams("task-1", "output", "missing.txt"),
    );
  ...
  it("returns 500 on non-ENOENT filesystem error", async () => {
    ...
    mockReadFile.mockRejectedValue(err);
    const res = await GET(
      makeReq("task-1", "output", "protected.txt"),
      makeParams("task-1", "output", "protected.txt"),
    );
  ...
  it("returns 400 for taskId containing '/'", async () => {
    const res = await GET(
      makeReq("task/evil", "output", "file.txt"),
      makeParams("task/evil", "output", "file.txt"),
    );
  ...
  it("returns 400 for taskId containing spaces", async () => {
    const res = await GET(
      makeReq("task id", "output", "file.txt"),
      makeParams("task id", "output", "file.txt"),
    );
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe("Invalid taskId");
  ...
  it("accepts valid taskId with alphanumeric, hyphens, underscores", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task_1-abc", "output", "file.txt"),
      makeParams("task_1-abc", "output", "file.txt"),
    );
    expect(res.status).toBe(200);
  });
  ...
  it("returns 400 for subfolder 'secrets'", async () => {
    const res = await GET(
      makeReq("task-1", "secrets", "file.txt"),
      makeParams("task-1", "secrets", "file.txt"),
    );
    expect(res.status).toBe(400);
    const body = await res.json();
  ...
  it("returns 400 for subfolder '..'", async () => {
    const res = await GET(
      makeReq("task-1", "..", "file.txt"),
      makeParams("task-1", "..", "file.txt"),
    );
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe("Invalid subfolder");
  ...
  it("accepts subfolder 'attachments'", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "attachments", "img.png"),
      makeParams("task-1", "attachments", "img.png"),
    );
    expect(res.status).toBe(200);
  });
  ...
  it("accepts subfolder 'output'", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "output", "result.json"),
      makeParams("task-1", "output", "result.json"),
    );
    expect(res.status).toBe(200);
  });
  ...
  it("returns 400 for filename containing '\\'", async () => {
    const res = await GET(
      makeReq("task-1", "output", "sub\\file.txt"),
      makeParams("task-1", "output", "sub\\file.txt"),
    );
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe("Invalid filename");
  ...
  it("returns application/octet-stream for unknown extension", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "output", "archive.xyz"),
      makeParams("task-1", "output", "archive.xyz"),
    );
    expect(res.status).toBe(200);
  ...
  it("returns application/octet-stream for file with no extension", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("task-1", "output", "Makefile"),
      makeParams("task-1", "output", "Makefile"),
    );
    expect(res.status).toBe(200);
    expect(res.headers.get("Content-Type")).toBe("application/octet-stream");
  });
  ...
  it("assembles correct file path from homedir + .nanobot/tasks/{taskId}/{subfolder}/{filename}", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    await GET(
      makeReq("my-task", "attachments", "doc.pdf"),
      makeParams("my-task", "attachments", "doc.pdf"),
    );
    expect(mockReadFile).toHaveBeenCalledWith(
      "/home/test/.nanobot/tasks/my-task/attachments/doc.pdf",
    );
  });
  ...
  it("returns correct Content-Type for .ts", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("t", "output", "index.ts"),
      makeParams("t", "output", "index.ts"),
    );
  ...
  it("returns correct Content-Type for .csv", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("t", "output", "data.csv"),
      makeParams("t", "output", "data.csv"),
    );
  ...
  it("returns correct Content-Type for .yaml", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("t", "output", "config.yaml"),
      makeParams("t", "output", "config.yaml"),
    );
  ...
  it("returns correct Content-Type for .svg", async () => {
    ...
    mockReadFile.mockResolvedValue(buf);
    const res = await GET(
      makeReq("t", "output", "icon.svg"),
      makeParams("t", "output", "icon.svg"),
    );
