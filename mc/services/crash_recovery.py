"""CrashRecoveryService — crash detection, retry policy, and escalation.

Extracted from mc/gateway.py AgentGateway class (Story 17.2, AC #2).
Preserves the exact retry semantics: one auto-retry (FR37), then crash (FR38).
"""
...
class CrashRecoveryService:
    """Handles agent crash detection, retry counting, and escalation.

    Preserves the exact semantics from the original AgentGateway:
    - First crash: transition to "retrying", post error message, re-dispatch.
    - Second crash (retry exhausted): transition to "crashed", post final error.
    ...
    async def _retry_task(
        ...
        logger.info(
            "[crash_recovery] Agent '%s' crashed on task %s. "
