...
class CCExecutorMixin:
    ...
    async def _execute_cc_task(
        ...
        try:
            ...
            await self._crash_task(
                task_id, title, f"Workspace preparation failed: {exc}", agent_name
            )
            return
        ...
        try:
            ...
            provider = ClaudeCodeProvider(
                cli_path=_cfg.claude_code.cli_path, defaults=_cfg.claude_code
            )
            prompt = f"{title}\n\n{description}" if description else title

            def on_stream(msg: dict) -> None:
                if msg.get("type") == "text":
                    create_background_task(self._post_cc_activity(task_id, agent_name, msg["text"]))
            ...
            await self._crash_task(
                task_id, title, f"Claude Code execution failed: {exc}", agent_name
            )
            return
        finally:
            if self._ask_user_registry is not None:
        ...
        if result.is_error:
            try:
                ...
                logger.warning(
                    "[executor] CC: output sync failed for errored task '%s'", title, exc_info=True
                )
            await self._crash_task(
                task_id, title, f"Claude Code error: {result.output[:1000]}", agent_name
            )
        else:
    ...
    async def _lookup_cc_session(self, agent_name: str, task_id: str) -> str | None:
        ...
        try:
            stored = await asyncio.to_thread(
                self._bridge.query, "settings:get", {"key": f"cc_session:{agent_name}:{task_id}"}
            )
    ...
    async def _store_cc_session(self, agent_name: str, task_id: str, session_id: str) -> None:
        ...
        try:
            await asyncio.to_thread(
                ...
                "settings:set",
                {"key": f"cc_session:{agent_name}:{task_id}", "value": session_id},
            )
            await asyncio.to_thread(
                self._bridge.mutation,
                "settings:set",
                {"key": f"cc_session:{agent_name}:latest", "value": session_id},
            )
            logger.info(
            ...
            logger.warning(
                "[executor] Failed to store CC session ID for %s", agent_name, exc_info=True
            )
    ...
    def _schedule_cc_consolidation(
        ...
        async def _post_cc_consolidate():
            try:
                ...
                if _model is None:
                    logger.warning(
                        "[executor] No model available for CC memory consolidation of '%s'", title
                    )
                    return
    ...
    async def _crash_task(
        self, task_id: str, title: str, error: str, agent_name: str = "System"
    ) -> None:
            ...
            if _tr_board_id:
                ...
                if _tr_board:
                    ...
                    if _tr_board_name:
                        ...
                        logger.info(
                            "[executor] CC thread reply: board '%s' (mode=%s)",
                            _tr_board_name,
                            _tr_memory_mode,
                        )
        except Exception:
            logger.warning(
                "[executor] CC thread reply: failed to resolve board workspace for task '%s'",
                task_id,
                exc_info=True,
            )
