...
DEFAULT_TASK_CONSOLIDATION_SYSTEM_PROMPT = (
    ...
    "Extract key facts from this task execution and produce a JSON response. "
    ...
    'Respond with ONLY a JSON object: {"history_entry": "<one-line summary for the history log>", '
    '"memory_update": "<full updated long-term memory as markdown, or empty string to leave unchanged>"}. '
...
async def consolidate_task_output(
    ...
    *,
    ...
    full_prompt = (system_prompt or DEFAULT_TASK_CONSOLIDATION_SYSTEM_PROMPT) + "\n\n" + prompt

    try:
        text = await run_utility_turn(full_prompt, tier="medium")
        args = extract_json(text)
    except Exception:
        logger.exception("Task memory consolidation utility turn failed for task %s", task_id)
        return False
